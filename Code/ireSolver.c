/* original ex12.c from:
   commit 95571756d2fe3fa1175454a522a4d94710fe89f7
   Author: Satish Balay <balay@mcs.anl.gov>
   Date:   Mon Sep 8 22:41:13 2014 -0500
...
typedef struct {
  ...
  PetscInt      NmaxTissue ;
  PetscScalar   *ElectricConductivity;  /* material property per tissue type */
  PetscScalar   voltage;                /* electrode voltage */
  /* Problem definition */
  BCType        bcType;
...
PetscErrorCode ProcessOptions(MPI_Comm comm, AppCtx *options)
{
  ...
  ierr = PetscMalloc(sizeof(PetscScalar)*options->NmaxTissue,&options->ElectricConductivity);CHKERRQ(ierr);
  ierr = PetscOptionsGetScalarArray(NULL,"-electric_conductivity" , options->ElectricConductivity, &options->NmaxTissue , NULL);CHKERRQ(ierr);
  ierr = PetscPrintf(PETSC_COMM_WORLD, "Electric Conductivity %f,%f,%f,%f,%f,%f,%f \n",
                                        options->ElectricConductivity[0],
                                        options->ElectricConductivity[1],
                                        options->ElectricConductivity[2],
                                        options->ElectricConductivity[3],
                                        options->ElectricConductivity[4],
                                        options->ElectricConductivity[5],
                                        options->ElectricConductivity[6]
                    );CHKERRQ(ierr);
  ...
  ierr = PetscOptionsScalar("-voltage", "voltage [Volts] ", "ex52.c", options->voltage, &options->voltage, NULL);CHKERRQ(ierr);
...
PetscErrorCode SetupMaterial(DM dm, DM dmAux, AppCtx *user)
{
  ...
  ierr = DMPlexProjectFunctionLocal(dmAux, matFuncs, ((void**)&user), INSERT_ALL_VALUES, nu);CHKERRQ(ierr);
...
int main(int argc, char **argv)
{
  ...
  ierr = PetscFree(user.ElectricConductivity);CHKERRQ(ierr);
  ierr = PetscFinalize();
  return 0;
}
