...
MANSEC           = SNES
EXAMPLESC	 = ex1.c ex2.c ex3.c  ex5.c ex5s.c ex7.c \
                ex10.c ex12.c ireSolver.c ex14.c ex15.c ex18.c ex19.c ex20.c ex21.c ex22.c \
                ex25.c ex28.c ex30.c ex33.c \
                ex35.c ex42.c ex46.c ex48.c \
                ex53.c ex54.c ex55.c ex58.c ex59.c ex60.c \
...
ex10: ex10.o chkopts
	...
	${RM} ex10.o
VTK_LIB = -rdynamic /usr/lib/libvtkVolumeRendering.so.5.8.0 /usr/lib/libvtkIO.so.5.8.0 /usr/lib/libvtkRendering.so.5.8.0 /usr/lib/libvtkIO.so.5.8.0 /usr/lib/libvtkGraphics.so.5.8.0 /usr/lib/libvtkImaging.so.5.8.0 /usr/lib/libvtkFiltering.so.5.8.0 /usr/lib/libvtkCommon.so.5.8.0 /usr/lib/libvtksys.so.5.8.0 /usr/lib/libvtkHybrid.so.5.8.0
ireSolver: ireSolver.o chkopts
	-${CLINKER} -o ireSolver ireSolver.o ${PETSC_SNES_LIB} ${VTK_LIB}
	${RM} ireSolver.o
ex12: ex12.o chkopts
...
runex19_14:
	-@${MPIEXEC} -n 4 ./ex19 -snes_monitor_short -pc_type mg -dm_mat_type baij -mg_coarse_pc_type bjacobi -da_refine 3 -ksp_type fgmres > ex19_14.tmp 2>&1; \
	   ...
	   ${RM} -f ex19_14.tmp
runex19_17:
	-@${MPIEXEC} -n 1 ./ex19 -snes_monitor_ksp -snes_monitor_short  -ksp_pc_side right  -snes_rtol 1.e-12 -ksp_rtol 1.e-12 > ex19_17.tmp 2>&1; \
	   if (${DIFF} output/ex19_17.out ex19_17.tmp) then true; \
	   else  printf "${PWD}\nPossible problem with ex19_17, diffs above\n=========================================\n"; fi; \
	   ${RM} -f ex19_17.tmp
runex19_18:
	-@${MPIEXEC} -n 1 ./ex19 -ksp_monitor_snes_lg -ksp_rtol 1.e-13 -snes_rtol 1.e-13 -ksp_pc_side right > ex19_18.tmp 2>&1; \
...
tags:
	ctags -R --languages=c,c++ --langmap=c++:+.txx --langmap=c++:+.cu --langmap=c++:+.cl $(PETSC_DIR)  /usr/include/vtk-5.8/ .
...
TESTEXAMPLES_C		       = ex1.PETSc runex1 runex1_2 ex1.rm ex2.PETSc runex2  runex2_3 ex2.rm ex3.PETSc runex3 \
                                 ...
                                 ex48.PETSc runex48 runex48_2 runex48_3 runex48_4 runex48_5 ex48.rm ex54.PETSc ex54.rm ex55.PETSc ex55.rm\
...
TESTEXAMPLES_SUITESPARSE       = ex19.PETSc runex19_klu runex19_klu_2 runex19_klu_3 ex19.rm
