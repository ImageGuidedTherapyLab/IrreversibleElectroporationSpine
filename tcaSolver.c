...
void nu_2d(const PetscReal x[], PetscScalar *u, void *ctx)
{
 // *u = x[0] + x[1];
  ...
  float p[3],d[3],v0[3]={0,0,0},v1[3]={0,0,0},v2[3]={0,0,0};
  ...
  //Points inside test
  if( user->selectEnclosedPoints->IsInsideSurface( x[0], x[1], x[2] ) )
  {
   *u = 57; 
  } 
  else{
...
//   myfile.open("newregister.stl",ios::in);
//   char str1[44],str2[44],str3[44],str4[44];
//   char str5[5];
//   char str6[1];
//   int i=0;
// 
...
// 
// 
//   myfile>>v0[0]>>v0[1]>>v0[2];
// //  cout<<v0<<"\n";
//   myfile>>tria>>v1[0]>>v1[1]>>v1[2];
// //  cout<<v1<<"\n";  
//   myfile>>tria>>v2[0]>>v2[1]>>v2[2];
// //  cout<<v2<<"\n";
//   }
// 
// 
...
PetscErrorCode ProcessOptions(MPI_Comm comm, AppCtx *options)
{
  ...
  std::string inputFilename("registerstlTestLarge.stl");
  vtkSmartPointer<vtkSTLReader> reader = vtkSmartPointer<vtkSTLReader>::New();
  reader->SetFileName(inputFilename.c_str());
  reader->Update();
  // initilize the bounding data structure
  options->selectEnclosedPoints = vtkSmartPointer<vtkSelectEnclosedPoints>::New();
  options->selectEnclosedPoints->Initialize( reader->GetOutput() );
...
PetscErrorCode SetupExactSolution(DM dm, AppCtx *user)
{
  ...
  case COEFF_FIELD:
    ...
    fem->g1Funcs[0] = NULL;
    fem->g2Funcs[0] = NULL;
    fem->g3Funcs[0] = g3_field_uu /*g3_field_uu*/;
    break;
...
#undef __FUNCT__
#define __FUNCT__ "SetupMultipleMaterialSection"
/*
  There is a problem here with uninterpolated meshes. The index in numDof[] is not dimension in this case,
  but sieve depth.

  Multiple field setup borrowed from 
  PETSc/petsc-dev/src/snes/examples/tutorials/ex62.c 
*/
PetscErrorCode SetupMultipleSection(DM dm, AppCtx *user)
{
  PetscSection    section;
